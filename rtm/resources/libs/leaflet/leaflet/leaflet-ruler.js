(function (factory, window) {
    ...
    L.Control.Ruler = L.Control.extend({
        ...
        onAdd: function (map) {
            // this._map = map;
            this.container = this.options.view;
            this.container.on('click', this._toggleMeasure, this);
            this._choice = false;
            var rtContainer = L.DomUtil.create('div', '');
            // rtContainer.classList.add('leaflet-ruler');
            this._defaultCursor = this._map._container.style.cursor;
            this._allLayers = L.layerGroup();
            return rtContainer;
        },
        onRemove: function () {
             this.container.un('click', this._toggleMeasure, this);
        },
        _toggleMeasure: function () {
            ...
            if (this._choice) {
                ...
                //   this.container.classList.add("leaflet-ruler-clicked");
                this._clickCount = 0;
                ...
                // this.container.classList.remove("leaflet-ruler-clicked");
        ...
        _clicked: function (e) {
            ...
            if (this._clickCount > 0) {
                ...
                if (this._clickCount > 1) {
                    text ='长度:' + '</b>&nbsp;' + this._totalLength.toFixed(this.options.lengthUnit.decimal) + '&nbsp;' + this.options.lengthUnit.display;
                }
                else {
                    text = '长度:' + '</b>&nbsp;' + this._result.Distance.toFixed(this.options.lengthUnit.decimal) + '&nbsp;' + this.options.lengthUnit.display;
                }
        ...
        _moving: function (e) {
            if (this._clickedLatLong) {
                ...
                if (this._clickCount > 1) {
                    text = '长度:' + '</b>&nbsp;' + this._addedLength.toFixed(this.options.lengthUnit.decimal) + '&nbsp;' + this.options.lengthUnit.display + '<br><div class="plus-length">(+' + this._result.Distance.toFixed(this.options.lengthUnit.decimal) + ')</div>';
                }
                else {
                    text = '长度:' + '</b>&nbsp;' + this._result.Distance.toFixed(this.options.lengthUnit.decimal) + '&nbsp;' + this.options.lengthUnit.display;
                }
